...
static JaneList<Token> *tokenize(Buf *buf, JaneList<char *> *include_paths,
                                 Buf *cur_dir_path) {
  Tokenize t = {0};
...
static int usage(char *arg0) {
  fprintf(stderr,
          "usage: %s --output outfile hello.jn\n--version (display version of "
          "jane language)\n-Ipath (add path to header include path)\n",
          arg0);
  return EXIT_FAILURE;
}

static void print_tokens(Buf *buf, JaneList<Token> *tokens) {
  for (int i = 0; i < tokens->length; i += 1) {
    Token *token = &tokens->at(i);
    printf("%s ", token_name(token));
    fwrite(buf_ptr(buf) + token->start_pos, 1,
           token->end_pos - token->start_pos, stdout);
    printf("\n");
  }
}
...
__attribute__((format(printf, 2, 3))) static void
ast_error(BuildAst *b, const char *format, ...) {
  int line = b->line + 1;
...
static const char *node_type_str(NodeType node_type) {
  ...
  jane_panic("akwoakow: unreachable");
}
...
static AstNode *build_ast(Buf *buf, JaneList<Token> *tokens) {
  BuildAst b = {0};
    ...
    case AstStateType:
      ...
      case TokenIdSymbol:
        ...
        if (mem_eql_str(token_mem, token_len, "u8")) {
          ...
          Buf msg = {0};
          buf_append_str(&msg, "invalid primitive data types '");
          buf_append_mem(&msg, token_mem, token_len);
          buf_append_str(&msg, "'");
          ast_error(&b, "%s", buf_ptr(&msg));
        }
...
int main(int argc, char **argv) {
  ...
  if (strcmp(in_files, "-") == 0) {
    ...
    if (!result) {
      jane_panic("unable to get current working dir: `%s`", strerror(errno));
    }
  ...
  fprintf(stderr, "----\n");
  fprintf(stderr, "%s\n", buf_ptr(in_data));
  JaneList<Token> *tokens = tokenize(in_data, &include_paths, cur_dir_path);
